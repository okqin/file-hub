...
async fn test_should_render_delete_available_action_only_with_delete_permission() -> Result<()> {
    ...
    assert!(body.contains("state.canDelete = Boolean(identity.actions.delete)"));
    assert!(body.contains("function appendDeleteAction(actions, resource, source)"));
    assert!(body.contains("if (!state.canDelete)"));
    assert!(body.contains("appendDeleteAction(actions, resource, 'directoryListing')"));
    assert!(body.contains("appendDeleteAction(actions, resource, 'serverSearch')"));
    Ok(())
}

#[tokio::test]
async fn test_should_confirm_directory_and_all_contents_before_browser_delete() -> Result<()> {
    let storage_root = tempfile::tempdir().context("create temporary storage root")?;
    let (app, _config, _config_dir) = app_from_storage_root(storage_root.path()).await?;
    ...
    assert!(body.contains("if (resource.kind === 'directory')"));
    assert!(body.contains(
        "window.confirm('Delete Directory \"' + resource.name + '\" and all contained Resources?')"
    ));
    assert!(!body.contains("resourceCount"));
    Ok(())
}

#[tokio::test]
async fn test_should_refresh_listing_preserve_sort_and_clear_search_after_delete() -> Result<()> {
    let storage_root = tempfile::tempdir().context("create temporary storage root")?;
    let (app, _config, _config_dir) = app_from_storage_root(storage_root.path()).await?;
    let response = app
        .oneshot(
    ...
    assert!(body.contains("fetch('/api/delete', {"));
    assert!(body.contains("if (source === 'serverSearch')"));
    ...
    assert!(body.contains("sort=' + encodeURIComponent(state.sort)"));
    assert!(body.contains("order=' + encodeURIComponent(state.order)"));
    Ok(())
}

#[tokio::test]
async fn test_should_rerun_same_server_search_after_delete() -> Result<()> {
    let storage_root = tempfile::tempdir().context("create temporary storage root")?;
    let (app, _config, _config_dir) = app_from_storage_root(storage_root.path()).await?;
    let response = app
        .oneshot(
