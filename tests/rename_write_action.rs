...
#[tokio::test]
async fn test_should_render_rename_available_action_only_with_rename_permission() -> Result<()> {
    let storage_root = tempfile::tempdir().context("create temporary storage root")?;
    let (app, _config, _config_dir) = app_from_storage_root(storage_root.path()).await?;
    ...
    let body = to_bytes(response.into_body(), usize::MAX)
        .await
        .context("read browser response")?;
    let body = String::from_utf8(body.to_vec()).context("browser response must be UTF-8")?;
    assert!(body.contains("id=\"rename-resource-dialog\""));
    assert!(body.contains("state.canRename = Boolean(identity.actions.rename)"));
    assert!(body.contains("function appendRenameAction(actions, resource, source)"));
    assert!(body.contains("if (!state.canRename)"));
    assert!(body.contains("appendRenameAction(actions, resource, 'directoryListing')"));
    assert!(body.contains("appendRenameAction(actions, resource, 'serverSearch')"));
    Ok(())
}

#[tokio::test]
async fn test_should_refresh_current_listing_preserve_sort_and_clear_search_after_rename()
-> Result<()> {
    let storage_root = tempfile::tempdir().context("create temporary storage root")?;
    let (app, _config, _config_dir) = app_from_storage_root(storage_root.path()).await?;
    let response = app
    ...
    assert!(body.contains("renameResourceForm.addEventListener('submit', submitRenameResource)"));
    assert!(body.contains("fetch('/api/rename', {"));
    assert!(body.contains("path: state.renameResource.resourcePath"));
    assert!(body.contains("newName: renameResourceName.value"));
    assert!(body.contains("renameResourceDialog.close()"));
    assert!(body.contains("resetAfterWrite()"));
...
#[tokio::test]
async fn test_should_rerun_same_server_search_after_rename() -> Result<()> {
    let storage_root = tempfile::tempdir().context("create temporary storage root")?;
    let (app, _config, _config_dir) = app_from_storage_root(storage_root.path()).await?;
    let response = app
        .oneshot(
    ...
    assert!(body.contains("var renameSource = state.renameSource"));
    assert!(body.contains("if (renameSource === 'serverSearch')"));
    assert!(body.contains("runServerSearch()"));
